...
async def force_rebalance(account_id: str, request: Request):
    ...
    manager = _get_manager(request)
    try:
        # 先尝试加载行情到引擎（引擎可能还没启动刷新循环，直接调用 get_all_quotes 走 storage 回退）
        # 这会填充 engine._quotes，后续 force_rebalance 通过 latest_quotes 属性读取
        engine = getattr(request.app.state, "engine", None)
        if engine is not None:
            try:
                bonds = await engine.get_all_quotes()
                if bonds:
                    logger.info(f"[ForceRebalance] Loaded {len(bonds)} bonds via get_all_quotes")
            except Exception as e:
                logger.warning(f"[ForceRebalance] get_all_quotes fallback failed: {e}")
