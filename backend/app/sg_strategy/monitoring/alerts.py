...
class NotificationConfig:
    ...
    # 邮件配置
    smtp_host: str = "smtp.sg-strategy.com"
    smtp_port: int = 587
    smtp_user: str = ""
    smtp_password: str = ""
    email_from: str = "alerts@sg-strategy.com"
...
class EmailNotifier:
    ...
    def _generate_text_content(self, alert: Alert) -> str:
        ...
        lines.append("")
        lines.append("---")
        lines.append("松岗量化可转债策略 V3.0 告警系统")
...
class SMSNotifier:
    ...
    def send(self, alert: Alert, recipients: List[str]) -> bool:
        ...
        # 短信内容（限制长度）
        content = f"【松岗策略】{alert.severity.value.upper()}:{alert.name[:50]}"
        if len(content) > 70:
            content = content[:67] + "..."
