...
class TdxAdapter:
            ...
            for i in range(0, len(codes_list), 80):
                ...
                try:
                    ...
                    for q in quotes:
                        ...
                        if not code:
                            continue
                        price_raw = float(q.get('price', 0) or 0)
                        last_close_raw = float(q.get('last_close', 0) or 0)
                        open_raw = float(q.get('open', 0) or 0)
                        high_raw = float(q.get('high', 0) or 0)
                        low_raw = float(q.get('low', 0) or 0)
                        vol = float(q.get('vol', 0) or 0)
                        amount = float(q.get('amount', 0) or 0)
                        # TDX 债券价格用 0.001元 存储（如 106798→106.80元），股票直接是元
                        divisor = 1.0
                        if price_raw > 1000 and (code.startswith("11") or code.startswith("12") or code.startswith("13") or code.startswith("118") or code.startswith("123")):
                            divisor = 100.0
                        price = price_raw / divisor
                        last_close = last_close_raw / divisor
                        open_p = open_raw / divisor
                        high = high_raw / divisor
                        low = low_raw / divisor
                        change_pct = ((price - last_close) / last_close * 100) if last_close > 0 else None
                        result[code] = {
                            "price": round(price, 2),
                            "open": round(open_p, 2),
                            "high": round(high, 2),
    ...
    def fetch_finance_info(self, code: str) -> Optional[dict]:
        ...
        try:
            ...
            jingzichan = _to_float(info.get('jingzichan'))
            meigujingzichan = _to_float(info.get('meigujingzichan'))
            zongguben = _to_float(info.get('zongguben'))
            jinglirun = _to_float(info.get('jinglirun'))
            zhuyingshouru = _to_float(info.get('zhuyingshouru'))
            zongzichan = _to_float(info.get('zongzichan'))
            liudongfuzhai = _to_float(info.get('liudongfuzhai'))
            gudongrenshu = _to_float(info.get('gudongrenshu'))
            liutongguben = _to_float(info.get('liutongguben'))
    ...
    def fetch_finance_batch(self, codes: list[str]) -> dict[str, dict]:
        ...
        for code in codes:
            ...
            if info and (info.get("pe") or info.get("pb") or info.get("eps")):
                result[code] = info
        return result
        ...
        try:
            ...
            for b in bars:
                ...
                result.append({
                    ...
                    "vol": _to_float(b.get('vol', 0)),
                    "amount": _to_float(b.get('amount', 0)),
                })
            self._last_used = time.time()
            return result
        except Exception as e:
        ...
        try:
            ...
            while offset < total and len(result) < count:
                ...
                for s in chunk:
                    ...
                    pre_close = _to_float(s.get('pre_close'))
                    if code:
                        result.append({
                            "code": code,
                            "name": name,
