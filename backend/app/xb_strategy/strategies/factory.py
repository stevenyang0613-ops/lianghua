...
class ConvertibleBondStrategy(BaseStrategy):
    ...
    def initialize(self):
        """初始化"""
        from app.xb_strategy.core.scoring import ScoringEngine
        from app.xb_strategy.core.filters import VetoFilter
        from app.xb_strategy.core.whitelist import WhitelistManager
        from app.xb_strategy.core.signals import SignalGenerator
...
class StrategyFactory:
    ...
    def _load_strategy(cls, strategy_id: str) -> Optional[Type[BaseStrategy]]:
        ...
        try:
            # 尝试从策略目录加载
            module_path = f"app.xb_strategy.strategies.{strategy_id}"
...
# 注册默认策略
StrategyFactory.register("xb_cb_default", ConvertibleBondStrategy)
