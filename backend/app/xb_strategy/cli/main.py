...
def init(aum: float, regime: str):
    ...
    strategy = XBConvertibleStrategy(aum, regime_map[regime])
...
def whitelist(top: int):
    """查看白名单"""
    strategy = XBConvertibleStrategy()
...
def cost(aum: float):
    """预估交易成本"""
    from app.xb_strategy.core.cost import TransactionCostModel
...
def timing(premium: float, amount: float, yield10y: float, pmi: float):
    """计算择时信号"""
    from app.xb_strategy.core.timing import TimingEngine, MarketData
...
def daily():
    """每日操作时间表"""
    click.echo("西部量化可转债策略 V3.0 每日操作时间表")
    click.echo("=" * 50)
...
def analyze(code: str):
    ...
    from app.xb_strategy.core.types import ConvertibleBondData
    from app.xb_strategy.config.weights import MarketRegime
...
def params():
    """显示策略参数"""
    from app.xb_strategy.config.settings import params as p

    click.echo("西部量化可转债策略 V3.0 核心参数")
    click.echo("=" * 50)
